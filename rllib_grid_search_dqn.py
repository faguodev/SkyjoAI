...
# Search spaces
tuning_stages = [
    # {"observation_mode": ["simple", "efficient_one_hot"]},
...
def train_model(
    ...
    if observation_mode == "onehot":
        ...
        pre_programmed_policy = PreProgrammedPolicyEfficientOneHot
    elif observation_mode == "simple":
        pre_programmed_policy = PreProgrammedPolicySimple

    model_config = {
        "custom_model": DQNActionMaskModel,
    ...
    config = (
        DQNConfig()
        .training(
            model=model_config,replay_buffer_config={
        ...
        .rollouts(num_rollout_workers=1, num_envs_per_worker=1, rollout_fragment_length=2)
    ...
    param_string = f"obs_{observation_mode}_indirect_{observe_other_player_indirect}__cr_{curiosity_reward}_ar_{action_reward_reduction}_decay_{action_reward_decay}_nn_{neural_network_size}"#_lr_{learning_rate}
    ...
    model_save_dir = f"trained_models/grid_search/dqn/{param_string}"
    os.makedirs(model_save_dir, exist_ok=True)
    max_iters = 2000

    #region Training
    
    logs_save_dir = f"logs/grid_search/dqn/{param_string}"
...
def evaluate_candidate(params):
    ...
    logs_path = f"logs/grid_search/dqn/{param_string}"
